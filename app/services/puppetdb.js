...
export class PuppetDB {
  ...
  get(path, params = {}) {
    ...
    return this.$http.get(`${this.serverUrl()}/${path}`, config);
  }
  ...
  handleResponse(promise, success) {
    promise.then(
      (resp) => { success(resp.data, resp.headers('X-Records')); },
      (resp) => {
        if (resp.status > 0) {
          throw new Error(`Failed to fetch ${resp.config.url} ${resp.status}: ${resp.statusText}`);
        }
      }
    );
  }
